...
TEST_SUITE("fpdb-store-server/DistributedTest" * doctest::skip(false)) {
  ...
  TEST_CASE("fpdb-store-server/DistributedTest/boot" * doctest::skip(false)) {
    ...
    // Start server 1
    auto server1 = Server::make(ServerConfig{"node1", 0, true, std::nullopt, 0,  0, "./cluster/node1/data"}, std::nullopt,
                                actor_manager);
    ...
    // Start server 2
    auto server2 = Server::make(ServerConfig{"node2", 0, false, std::nullopt, std::nullopt, 0, "./cluster/node2/data"},
