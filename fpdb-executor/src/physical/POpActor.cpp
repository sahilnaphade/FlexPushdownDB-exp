...
void POpActor::on_regular_message(const fpdb::executor::message::Envelope &msg) {
  if (msg.message().type() == MessageType::TUPLESET_READY_FPDB_STORE) {
    const auto &typedMessage = dynamic_cast<const TupleSetReadyFPDBStoreMessage &>(msg.message());
    read_table_from_fpdb_store(typedMessage.getHost(), typedMessage.getPort(), typedMessage.sender());
    return;
  }
...
void POpActor::read_table_from_fpdb_store(const std::string &host, int port, const std::string &sender) {
  ...
  auto ticketObj = fpdb::store::server::flight::GetTableTicket::make(opBehaviour_->getQueryId(), opBehaviour_->name());
  ...
  std::shared_ptr<Message> tupleSetMessage = std::make_shared<TupleSetMessage>(TupleSet::make(table), sender);
