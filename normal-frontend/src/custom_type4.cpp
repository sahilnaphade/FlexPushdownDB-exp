...
//  friend class rectangle;
  ...
  template <class Inspector>
  friend bool inspect(Inspector& f, shape& x) {
...
public:
  ...
  rectangle(point top_left, point bottom_right, int id)
          : shape(id), top_left_(top_left), bottom_right_(bottom_right) {
    // nop
  }
  ...
  friend bool inspect(Inspector& f, rectangle& x) {
    return f.object(x).fields(f.field("id", x.getId()),
                              f.field("top-left", x.top_left_),
                              f.field("bottom-right", x.bottom_right_));
  }

  static auto make(point top_left, point bottom_right, int id) {
    return std::make_shared<rectangle>(top_left, bottom_right, id);
  }
...
public:
  ...
  circle(point center, int32_t radius, int id) : shape(id), center_(center), radius_(radius) {
    // nop
  }
  ...
  static auto make(point center, int32_t radius, int id) {
    return std::make_shared<circle>(center, radius, id);
  }
